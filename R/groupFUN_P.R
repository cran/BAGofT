################################################################
#Based on the selected partition from splitFUN_P, group the data
#in the test set datE. Return the group indices for each observation
################################################################


#spR : result from splitFUN_P
#datE : test data

groupFUN_P <- function(spR, datE, phatE){
  # add the fitted probability in the test data into
  # the data frame
  datE$phat <- phatE
